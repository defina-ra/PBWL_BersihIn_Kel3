...
<div class="space-y-5 pb-6">
    ...
    <div class="hero-card">
        ...
        <h2 class="text-xl font-bold mb-3">{{ $booking->service_name }}</h2>
        <div class="flex gap-4 text-sm">
            <div>
                <p class="text-white/60 text-xs">No. Pesanan</p>
                <p class="text-white font-semibold">#{{ str_pad($booking->id, 5, '0', STR_PAD_LEFT) }}</p>
            </div>
            <div class="w-px bg-white/20"></div>
            <div>
                <p class="text-white/60 text-xs">Tanggal</p>
                <p class="text-white font-semibold">
                    {{ $booking->booking_date ? \Carbon\Carbon::parse($booking->booking_date)->format('d M Y') : \Carbon\Carbon::parse($booking->created_at)->format('d M Y') }}
                </p>
            </div>
            <div class="w-px bg-white/20"></div>
            <div>
        ...
        <form action="/bersihin/customer/ulasan" method="POST">
            ...
            <div class="mb-5">
                ...
                <div class="flex flex-wrap gap-2" id="tagGroup">
                    @foreach(['Petugas ramah', 'Hasil bersih', 'Tepat waktu', 'Harga terjangkau', 'Komunikatif', 'Profesional'] as $tag)
                    <button type="button"
            ...
            <div class="mb-6">
                ...
                <textarea name="comment" id="commentBox" class="review-input" rows="4"
                    placeholder="Tulis ulasan kamu di sini... (opsional)">{{ old('comment') }}</textarea>
