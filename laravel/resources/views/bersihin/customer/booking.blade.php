...
@section('page-subtitle', 'Isi detail pesanan kebersihan kamu')
    ...
    <form action="/bersihin/booking" method="POST" id="bookingForm">
        ...
        <div class="form-card mb-4">
            ...
            <div class="space-y-3" id="serviceList">
                ...
                <div class="service-card {{ isset($service) && $service->id == $l->id ? 'selected' : '' }}"
                     onclick="selectService({{ $l->id }}, '{{ $l->service_name }}', {{ $l->price }}, this)">
                    <div class="check">✓</div>
                    <div class="flex justify-between items-start">
                        <div>
                            <p class="font-bold text-gray-800 text-sm">{{ $l->service_name }}</p>
                            <p class="text-gray-400 text-xs mt-0.5">{{ $l->description ?? 'Layanan kebersihan profesional' }}</p>
                            @if($l->duration)
                            <p class="text-gray-400 text-xs mt-0.5">⏱ {{ $l->duration }} menit</p>
                            @endif
                        </div>
                        <div class="text-right flex-shrink-0 ml-3">
                            <p class="font-bold text-blue-600 text-sm">Rp {{ number_format($l->price, 0, ',', '.') }}</p>
                        </div>
                    </div>
                </div>
                @endforeach
        ...
        <div class="form-card mb-4">
            ...
            <div>
                ...
                <div class="time-grid">
                    @foreach(['07:00', '08:00', '09:00', '10:00', '11:00', '13:00', '14:00', '15:00', '16:00'] as $time)
                    <button type="button" class="time-btn {{ old('booking_time') == $time ? 'selected' : '' }}"
...
let selectedTime = '{{ old('booking_time') }}';
