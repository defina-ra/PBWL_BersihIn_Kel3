...
<div class="space-y-4">
    ...
    <div class="order-card" data-status="{{ in_array($p->status, ['done']) ? 'selesai' : ($p->status === 'pending' ? 'pending' : 'proses') }}">
        <div class="p-6">
            ...
            <div class="flex justify-between items-start mb-2">
                ...
                <p class="text-lg font-extrabold text-green-700 flex-shrink-0">Rp {{ number_format($p->price, 0, ',', '.') }}</p>
            </div>
            ...
            <div class="flex items-center justify-between mb-3">
                <div class="flex items-center gap-1.5 text-xs text-gray-500">
                    ...
                    {{ Str::limit($p->address, 35) }}
                </div>
                <div class="text-xs text-right">
                    @if($p->petugas_name)
                    <p class="text-gray-500">Petugas: <span class="font-semibold text-gray-700">{{ $p->petugas_name }}</span></p>
                    @else
            ...
            <div class="flex items-center gap-2 px-4 py-3 rounded-xl mb-3" style="background:#ede9fe;">
                ...
                <div>
                    ...
                    <p class="text-xs text-purple-500">{{ $p->petugas_name ?? 'Petugas' }} sedang menuju lokasimu. Harap siapkan akses masuk.</p>
                </div>
            </div>
            @elseif($p->status === 'in_progress')
            <div class="flex items-center gap-2 px-4 py-3 rounded-xl mb-3" style="background:#fef3c7;">
                ...
                <div>
                    ...
                    <p class="text-xs text-yellow-600">{{ $p->petugas_name ?? 'Petugas' }} sedang membersihkan. Mohon jangan ganggu proses pengerjaan.</p>
                </div>
            </div>
            @elseif($p->status === 'confirmed' && $p->petugas_name)
            <div class="flex items-center gap-2 px-4 py-3 rounded-xl mb-3" style="background:#eff6ff;">
                ...
                <div>
                    ...
                    <p class="text-xs text-blue-500">{{ $p->petugas_name }} akan segera menuju lokasimu sesuai jadwal.</p>
                </div>
            </div>
            ...
            <div class="flex justify-end items-center gap-2 border-t border-gray-100 pt-4">
                <button class="btn-outline"
                    onclick="lihatDetail({
                        ...
                        status: '{{ $p->status }}',
                        date: '{{ \Carbon\Carbon::parse($p->booking_date)->format('d M Y') }}',
                        time: '{{ $p->booking_time }}',
                        address: '{{ addslashes($p->address) }}',
                        price: '{{ number_format($p->price, 0, ',', '.') }}',
                        petugas: '{{ $p->petugas_name ?? 'Belum ditugaskan' }}'
                    })">
                    Lihat Detail
                </button>
                @if($p->status === 'done')
                ...
                @elseif(in_array($p->status, ['confirmed', 'on_the_way', 'in_progress']))
                <button class="btn-solid"
                    onclick="hubungiPetugas('{{ $p->petugas_name ?? '-' }}', '{{ $p->petugas_phone ?? '-' }}', '{{ $p->service_name }}')">
                    Hubungi Petugas
                </button>
                @elseif($p->status === 'pending')
                <a href="/bersihin/pembayaran?booking_id={{ $p->id }}"
