        ...
        <div class="bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-sm transition">
            <div class="h-36 bg-emerald-50 flex items-center justify-center relative">
                ...
                <form method="POST" action="/bersihin/admin/layanan/hapus"
                      onsubmit="return confirm('Hapus layanan {{ $l->service_name }}?')"
            ...
            <div class="p-4">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <button onclick="bukaModalEdit({{ $l->id }}, '{{ addslashes($l->service_name) }}', '{{ addslashes($l->description) }}', {{ $l->price }}, {{ $l->duration ?? 60 }})"
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50/60 transition">
                    <td class="px-5 py-4">
                        <p class="text-sm font-semibold text-gray-800">{{ $pr->title }}</p>
                        <p class="text-xs text-gray-400 mt-0.5">{{ $pr->subtitle }}</p>
                    </td>
                    <td class="px-5 py-4">
                    ...
                    <td class="px-5 py-4">
                        <span class="text-sm text-gray-600">{{ $pr->valid_until }}</span>
                    </td>
                    <td class="px-5 py-4">
                        <span class="text-xs font-semibold {{ $pr->is_active ? 'text-emerald-600' : 'text-gray-400' }}">
