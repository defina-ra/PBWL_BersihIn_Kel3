...
<div class="grid grid-cols-4 gap-4 mb-6">
    ...
    <div class="bg-white rounded-xl border border-gray-100 px-5 py-4">
        <div class="flex items-start justify-between mb-3">
            ...
            <span class="text-xs font-semibold text-red-500 bg-red-50 px-2 py-0.5 rounded-full">{{ $perluVerifikasi }}</span>
        </div>
        <p class="text-xs text-gray-400 font-medium uppercase tracking-wide">Perlu Verifikasi</p>
        <p class="text-2xl font-extrabold text-gray-900 mt-1">{{ $perluVerifikasi }} Transaksi</p>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-100 px-5 py-4">
        ...
        <p class="text-2xl font-extrabold text-gray-900 mt-1">{{ $belumDijadwalkan }} Pesanan</p>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-100 px-5 py-4">
        ...
        <p class="text-2xl font-extrabold text-gray-900 mt-1">{{ $petugasAktif }} Orang</p>
    </div>
...
<div class="grid grid-cols-3 gap-4 mb-6">
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-5">
        ...
        <div class="space-y-3">
            ...
            @forelse($daftarPetugas as $index => $p)
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2.5">
                    <div class="w-8 h-8 {{ $colors[$index % count($colors)] }} rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0">
                        {{ strtoupper(substr($p->name, 0, 2)) }}
                    </div>
                    <div>
                        <p class="text-sm font-semibold text-gray-800 leading-none">{{ $p->name }}</p>
    ...
    <table class="w-full">
        ...
        <tbody class="divide-y divide-gray-50">
            ...
            <tr class="hover:bg-gray-50/50">
                <td class="px-5 py-3.5 text-sm font-semibold text-[#064E3B]">#BRS-{{ $p->id }}</td>
                <td class="px-5 py-3.5 text-sm text-gray-700">{{ $p->customer_name }}</td>
                <td class="px-5 py-3.5">
                    <p class="text-sm font-semibold text-gray-800">{{ $p->service_name }}</p>
                ...
                <td class="px-5 py-3.5">
                    ...
                    @else
                        <span class="text-xs font-semibold text-gray-500 bg-gray-50 px-3 py-1 rounded-full">{{ $p->status }}</span>
                    @endif
                </td>
                <td class="px-5 py-3.5">
