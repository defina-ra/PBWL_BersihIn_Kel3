...
<div class="grid grid-cols-4 gap-4 mb-6">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 px-5 py-4">
        ...
        <p class="text-3xl font-extrabold text-gray-900 mt-1">{{ $totalSelesai }}</p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 px-5 py-4">
        ...
        @php $avgRating = $riwayat->whereNotNull('rating')->avg('rating'); @endphp
        <p class="text-3xl font-extrabold text-gray-900 mt-1">{{ $avgRating ? number_format($avgRating, 1) : '-' }} <span class="text-base font-semibold text-gray-400">/5</span></p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 px-5 py-4">
        ...
        <p class="text-3xl font-extrabold text-gray-900 mt-1">{{ $riwayat->whereNotNull('rating')->count() }}</p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-5">
        ...
        <div class="space-y-3">
            ...
            <div class="flex items-center gap-3 p-3 bg-blue-50 rounded-xl border border-blue-100">
                ...
                <div>
                    <p class="text-xs font-bold text-blue-700">{{ $totalSelesai }} Tugas Selesai</p>
...
<div class="bg-white rounded-2xl border border-gray-100">
    ...
    <table class="w-full">
        ...
        <tbody class="divide-y divide-gray-50">
            ...
            <tr class="hover:bg-gray-50/60 transition">
                <td class="px-5 py-4">
                    <p class="text-sm font-semibold text-gray-800">{{ \Carbon\Carbon::parse($r->booking_date)->format('d M Y') }}</p>
                    <p class="text-xs text-gray-400 mt-0.5">{{ $r->booking_time }}</p>
                </td>
                <td class="px-5 py-4">
                    <div class="flex items-center gap-2.5">
                        <div class="w-8 h-8 bg-emerald-500 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
                            {{ strtoupper(substr($r->customer_name, 0, 2)) }}
                        </div>
                        <span class="text-sm font-semibold text-gray-800">{{ $r->customer_name }}</span>
                    </div>
                </td>
                <td class="px-5 py-4">
                ...
                <td class="px-5 py-4 max-w-xs">
                    <p class="text-sm text-gray-500 italic truncate">"{{ $r->comment ?? 'Belum ada ulasan' }}"</p>
                </td>
                <td class="px-5 py-4">
    ...
    <div class="flex items-center justify-between px-5 py-4 border-t border-gray-100">
        <p class="text-xs text-gray-400">Menampilkan {{ $riwayat->count() }} pesanan</p>
    </div>
